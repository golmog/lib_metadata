            ...
            try:
                ...
                if not skip_default_poster_logic:
                    ...
                    if ps_to_poster_setting and ps_url_from_search_cache:
                        resolved_poster_url_step1 = ps_url_from_search_cache
                    # 2. PL 크롭 설정
                    ...
                    elif pl_url_detail_page_default and ps_url_from_search_cache:
                        loc = SiteUtil.has_hq_poster(ps_url_from_search_cache, pl_url_detail_page_default, proxy_url=proxy_url)
                        if loc:
                            resolved_poster_url_step1 = pl_url_detail_page_default
                            resolved_crop_mode_step1 = loc
                        elif SiteUtil.is_hq_poster(ps_url_from_search_cache, pl_url_detail_page_default, proxy_url=proxy_url):
                            resolved_poster_url_step1 = pl_url_detail_page_default
                    # 4. PS만 존재 시 (다른 설정 없을 때, 아트 비교 전 임시) - 사실상 이 조건은 아트 비교 후 fallback에서 처리
                    elif ps_url_from_search_cache and not pl_url_detail_page_default and not crop_mode_setting and not ps_to_poster_setting:
                        pass # 아트 비교로 넘어감
                    ...
                    if not resolved_poster_url_step1 and arts_urls_page_default and not ps_to_poster_setting:
                        specific_art_candidates_for_class = []
                        ...
                        if ps_url_from_search_cache:
                            for sp_candidate_for_class in specific_art_candidates_for_class:
                                if SiteUtil.is_hq_poster(ps_url_from_search_cache, sp_candidate_for_class, proxy_url=proxy_url):
                    ...
                    if not resolved_poster_url_step1 and ps_url_from_search_cache:
                    ...
                    if resolved_poster_url_step1 == ps_url_from_search_cache and \
                       is_vr_content and \
                       not ps_to_poster_setting and \
                       ps_url_from_search_cache:
                        skip_mgs_for_vr_and_use_ps = True
                        logger.info(f"MGStage ({cls.module_char}): VR content ('{title_for_vr_check}') and general logic chose PS ('{ps_url_from_search_cache}'). Skipping MGS style processing.")
                    else:
                        logger.debug(f"MGStage ({cls.module_char}): VR skip condition not met. ResolvedPoster='{resolved_poster_url_step1}', PS='{ps_url_from_search_cache}', isVR='{is_vr_content}', ps_to_poster='{ps_to_poster_setting}'")
                    ...
                    if not skip_mgs_for_vr_and_use_ps:
                        ...
                        if pl_url_detail_page_default and ps_url_from_search_cache and \
                           not ps_to_poster_setting and resolved_poster_url_step1 == ps_url_from_search_cache:
                            ...
                            logger.debug(f"MGStage ({cls.module_char}): MGS special processing conditions not met. PL='{pl_url_detail_page_default}', PS='{ps_url_from_search_cache}', ps_to_poster='{ps_to_poster_setting}', resolved_poster='{resolved_poster_url_step1}'")
                        ...
                        if attempt_mgs_special_local:
                            ...
                            temp_filepath, _, _ = SiteUtil.get_mgs_half_pl_poster_info_local(ps_url_from_search_cache, pl_url_detail_page_default, proxy_url=proxy_url)
...
class SiteMgstageAma(SiteMgstage):
    module_char = "D"
    search_module_query = "&type=amateur"
