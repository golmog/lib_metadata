...
class SiteJavbus(object):
    ...
    def search(
        ...
        try:
            ...
            url = "{site_base_url}/search/{keyword}".format(
                site_base_url=cls.site_base_url, keyword=keyword
            )
    ...
    def info(cls, code, do_trans=True, proxy_url=None, image_mode="0"):
        try:
            ret = {}
            url = "{site_base_url}/{code}".format(
                site_base_url=cls.site_base_url, code=code[2:]
            )
            ...
            data = SiteUtil.get_image_url(
                img_url, image_mode, proxy_url=proxy_url, with_poster=True
            )
            ...
            tagline = (
                tree.xpath("/html/body/div[5]/h3/text()")[0]
                .lstrip(entity.title)
                .strip()
            )
            entity.tagline = (
                SiteUtil.trans(tagline, do_trans=do_trans)
            ...
            entity.fanart = []
            for tag in tags:
                entity.fanart.append(
                    SiteUtil.process_image_mode(
                        image_mode, tag.attrib["href"], proxy_url=proxy_url
                    )
                )
