...
class SiteAvdbs(object):
    ...
    def get_actor_info(entity_actor, proxy_url=None, retry=True):
        try:
            seq_url = (
                "https://www.avdbs.com/w2017/api/iux_kwd_srch_log.php?op=srch&kwd=%s"
            ...
            try:
                ...
                url = (
                    "https://www.avdbs.com/w2017/page/search/search_actor.php?kwd=%s&seq=%s"
                    % (entity_actor["originalname"], seq_val)
                )
                logger.debug(url)
                res = requests.get(
                    url, headers=SiteUtil.default_headers, proxies=proxies, timeout=5
                )
            ...
            if img_tag:
                ...
                if (
                    tmp.split("(")[1].split(")")[0]
                    or tmp.split("（")[1].split("）")[0] == entity_actor["originalname"]
                ):
                    entity_actor["name"] = (
                        nodes[0].xpath("./a")[0].text_content().strip()
                    )
                    entity_actor["name2"] = (
                        nodes[1].xpath("./a")[0].text_content().strip().split("(")[0]
                    )
                    entity_actor["site"] = "avdbs"
                    entity_actor["thumb"] = SiteUtil.process_image_mode(
                        "3", img_tag[0].attrib["src"].strip()
                    )
                else:
            ...
            if retry:
                ...
                time.sleep(2)
                return SiteAvdbs.get_actor_info(
